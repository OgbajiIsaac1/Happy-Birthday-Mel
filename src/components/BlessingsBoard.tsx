...
export default function BlessingsBoard() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) {
      setErrorMsg('Please enter a blessing.');
      return;
    }

    if (trimmed.length < 2) {
      setErrorMsg('Too short! Try a full word.');
      return;
    }

    if (trimmed.length > 25) {
      setErrorMsg('Please keep the blessing under 25 characters.');
      return;
    }
          ...
          {loading ? (
            ...
            <AnimatePresence>
              {blessings.map((b, index) => {
                // Determine a deterministic class color based on character codes
                ...
                return (
                  <motion.span
                    ...
                    className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border shadow-md flex items-center space-x-1.5 transition-all duration-300 ${tagColorClass}`}
                  >
        ...
        <form onSubmit={handleSubmit} className="max-w-md mx-auto flex flex-col sm:flex-row gap-3">
          <div className="flex-grow space-y-1">
            <input
              ...
              placeholder="e.g. Grace, Joy, Favor, Success..."
              disabled={submitting}
              maxLength={25}
              required
            ...
            />
            {errorMsg && (
              <p className="text-[10px] text-red-400 font-semibold pl-1">{errorMsg}</p>
            )}
          </div>
